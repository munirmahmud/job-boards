...
const JobList = ({job}) => {
    ...
    return (
        <div className={`flex flex-col bg-white shadow-md my-16 mx-10 p-6 rounded ${job.featured && 'border-l-4 border-teal-400 border-solid'} md:flex-row`}>
            ...
            <div className="flex items-center mt-4 pt-4 border-t border-gray-500 border-solid md:ml-auto md:border-0 md:mt-0 md:pt-0">
                {langAndTools ? langAndTools.map((langAndTool, index) => <span key={index} className="bg-teal-100 text-teal-600 font-bold mr-4 px-3 py-2 rounded cursor-pointer">{langAndTool}</span>) : ''}
            </div>
        </div>
    )
}
